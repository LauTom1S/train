 ...
 *
 * @return Тип: int. Код завершения программы.
 * Возможные значения:
 * - 0 — программа завершена успешно;
 * Программы обычно возвращают 0 при успешном завершении.
 */
int main()
{
    ...
    wstring date;                              ///< Временная переменная для даты (в коде не используется).
    ...
    _setmode(_fileno(stdout), _O_U16TEXT); ///< Установка режима stdout для вывода Unicode (UTF-16).
    _setmode(_fileno(stdin), _O_U16TEXT);  ///< Установка режима stdin для ввода Unicode (UTF-16).
